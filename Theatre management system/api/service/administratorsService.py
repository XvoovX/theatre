...
class AdministratorsService(AdministratorsController):
    ...
    def login(cls, **kwargs):
        ...
        try:
            administrators_info = db.session.query(
                cls.Account,
                cls.Password,
            ) \
            ...
            if not administrators_info:
                return {'code': RET.LOGINERR, 'message': error_map_EN[RET.LOGINERR], 'error': "用户或密码错误"}

            administrators_info = dict(administrators_info._asdict())

            if not administrators_info['Password'] == password:
                return {'code': RET.LOGINERR, 'message': error_map_EN[RET.LOGINERR], 'error': "用户或密码错误"}

            db.session.close()

            print(administrators_info)
            back_dict = {
                "account": administrators_info['Account'],
            }
